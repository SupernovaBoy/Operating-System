...
void sort_process(Process* process_array, int process_num);
...
void PS(Process* process_array, int process_num);



/*
SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY); 无色
...
void print_process(Process* process_array, int process_num) {
	...
	cout << "进程名       创建时间       运行时间       优先级       完成时间       周转时间       带权周转时间" << endl;
	for (int i = 0; i < process_num; i++) {
...
void FCFS(Process* process_array, int process_num) {
	sort_process(process_array, process_num);
	for (int i = 0; i < process_num; i++) {
...
void SJF(Process* process_array, int process_num) {
	...
	int execute_time_sum = process_array[0].execute_time;
...
void PS(Process* process_array, int process_num) {
	...
	for (int i = 1; i < process_num; i++) {
		for (int j = i + 1; j < process_num; j++) {
			if (process_array[j].priority > process_array[i].priority \
				&& process_array[j].create_time < process_array[i].complete_time) {
				Process temp = process_array[i];
				process_array[i] = process_array[j];
				process_array[j] = temp;
			}
